  ...
  return (
    <aside className={`sidebar ${isOpen ? '' : 'collapsed'}`} id="sidebar">
      <div className="sidebar-logo">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s' }}>
      ...
      <nav className="sidebar-nav">
        <div className="sidebar-section">
          ...
          <NavLink to="/orders" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} style={{ textDecoration: 'none' }}>
            ...
            {isOpen && <span className="nav-label">All Orders</span>}
          </NavLink>
          <NavLink to="/my-orders" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} style={{ textDecoration: 'none' }}>
            ...
            {isOpen && <span className="nav-label">My Orders</span>}
          </NavLink>
        </div>
        <div className="sidebar-section">
      ...
      <div className="sidebar-footer">
        <div className="sidebar-user">
          ...
          <button className="sidebar-toggle" onClick={() => setIsOpen(!isOpen)} title={isOpen ? "Collapse sidebar" : "Expand sidebar"}>
            <svg 
              width="14" 
              height="14" 
              viewBox="0 0 24 24" 
              ...
              stroke="currentColor" 
              strokeWidth="2"
              style={{ transform: isOpen ? 'none' : 'rotate(180deg)', transition: 'transform 0.3s' }}
            >
