      ...
      <div style={{
        ...
        <div style={{ padding: '8px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {(() => { const sz = attId != null ? portalAttachments?.find(a => a.id === attId)?.file_size : undefined; return sz ? <span style={{ fontSize: 11, color: '#9CA3AF', flexShrink: 0, whiteSpace: 'nowrap' }}>{formatBytes(sz)}</span> : null })()}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{fileName}</div>
          {(() => { const sz = attId != null ? portalAttachments?.find(a => a.id === attId)?.file_size : undefined; return sz ? <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 1 }}>{formatBytes(sz)}</div> : null })()}
        </div>
        <button onClick={(e) => { e.stopPropagation(); handleDownload() }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6366F1', lineHeight: 1, flexShrink: 0, padding: 0 }} title="Download">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
        </button>
...
function TimelineEvent({ event, isOptimistic, onRetry, onDelete, onEdit, onReply, onHighlightQuoted, onHighlightPortalMsg, orderId, portalAttachments, portalMessages, portalAttCaptions, quotedEvent, currentUserId }: {
  ...
  switch (event.type) {
    ...
    case 'due_date_changed':
      ...
      text = event.payload.to ? `Due date set to ${formatDate(event.payload.to)}` : 'Due date removed'
      break
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, background: '#F5F6FA', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          {order.order_source && (
            <PanelSection label="Order Source">
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: '#F1F5F9', borderRadius: 6, border: '1px solid #E2E8F0', fontSize: 13, fontWeight: 600, color: '#475569', textTransform: 'capitalize' }}>
                ...
                {order.order_source === 'b2b' && (
                  <svg viewBox="0 0 512 512" width="14" height="14" style={{ marginRight: 2 }}>
                    <g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32">
                ...
                {order.order_source === 'store' && (
                  <svg viewBox="0 0 512 512" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32" width="14" height="14" style={{ marginRight: 2 }}>
                    <line x1="448" y1="448" x2="448" y2="240"/>
                    <line x1="64" y1="240" x2="64" y2="448"/>
                    <path d="M382.47,48H129.53C107.74,48,88.06,60,79.6,78.46L36.3,173c-14.58,31.81,9.63,67.85,47.19,69q1,0,2,0c31.4,0,56.85-25.18,56.85-52.23,0,27,25.46,52.23,56.86,52.23S256,218.62,256,189.77c0,27,25.45,52.23,56.85,52.23s56.86-23.38,56.86-52.23c0,28.85,25.45,52.23,56.85,52.23q1,0,1.95,0c37.56-1.17,61.77-37.21,47.19-69L432.4,78.46C423.94,60,404.26,48,382.47,48Z"/>
                    <line x1="32" y1="464" x2="480" y2="464"/>
                    <path d="M136,288h80a24,24,0,0,1,24,24v88a0,0,0,0,1,0,0H112a0,0,0,0,1,0,0V312A24,24,0,0,1,136,288Z"/>
                    <path d="M288,464V312a24,24,0,0,1,24-24h64a24,24,0,0,1,24,24V464"/>
                  </svg>
                )}
