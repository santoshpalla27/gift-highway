...
function TimelineEvent({ event, isOptimistic }: { event: OrderEvent; isOptimistic?: boolean }) {
  const isComment = event.type === 'comment_added'

  if (isComment) {
    return (
      <div style={{ display: 'flex', gap: 10, opacity: isOptimistic ? 0.6 : 1 }}>
        <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 11, color: '#9CA3AF' }}>{timeAgo(event.created_at)}</span>
          </div>
          <div style={{
  ...
  switch (event.type) {
    ...
    case 'status_changed':
      ...
      text = `changed status: ${STATUS_META[event.payload.from]?.label ?? event.payload.from} → ${STATUS_META[event.payload.to]?.label ?? event.payload.to}`
      break
    case 'priority_changed':
      icon = <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
      text = `changed priority: ${event.payload.from} → ${event.payload.to}`
      break
    case 'due_date_changed':
      ...
      text = event.payload.to ? `set due date to ${event.payload.to}` : `removed due date`
      break
    ...
    default:
      ...
      text = `updated order details`
  }

  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
      <div style={{
        width: 28, height: 28, borderRadius: '50%', background: '#F3F4F6', color: '#6B7280',
      ...
      <div style={{ paddingTop: 4 }}>
        <span style={{ fontSize: 13, color: '#374151' }}>
          <span style={{ fontWeight: 600, color: '#111827' }}>{event.actor_name}</span>{' '}{text}
        </span>
        <span style={{ fontSize: 11, color: '#9CA3AF', marginLeft: 8 }}>{timeAgo(event.created_at)}</span>
      </div>
    </div>
  )
}
...
export function OrderDetailPage() {
  ...
  // Clear optimistic entries once the real ones appear
  useEffect(() => {
            ...
            {eventsLoading ? (
              ...
              allEvents.map((ev, i) => (
                <div key={ev.id} style={{ animation: i >= events.length - 3 ? 'fadeSlideIn 0.25s ease' : 'none' }}>
                  <TimelineEvent event={ev} isOptimistic={ev.id.startsWith('opt-')} />
                </div>
              ))
            )}
            <div ref={feedEndRef} />
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: commentText.trim() ? '#6366F1' : '#E4E6EF',
