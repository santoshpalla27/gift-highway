    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(17, 24, 39, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, animation: 'fadeIn 0.2s ease-out' }}>
      ...
      <div className="premium-modal">
        <div className="premium-modal-header">
          <div>
            <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#111827', margin: 0, letterSpacing: '-0.02em' }}>Add New User</h2>
            <p style={{ fontSize: '13px', color: '#6B7280', margin: '4px 0 0 0' }}>Create an account and assign roles.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input type="text" placeholder="Jane Doe" className="premium-input" {...register('name')} />
                {errors.name && <p style={{ color: '#EF4444', fontSize: '12px', marginTop: '6px' }}>{errors.name.message}</p>}
              </div>

              <div style={{ gridColumn: '1 / -1' }}>
                <label className="premium-label">Email Address</label>
                <input type="email" placeholder="jane@company.com" className="premium-input" {...register('email')} />
