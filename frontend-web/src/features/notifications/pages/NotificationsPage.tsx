...
export function NotificationsPage() {
  ...
  const filtered = useMemo(() => {
    return allEvents.filter(e => {
      ...
      if (dateFrom) {
        if (new Date(e.created_at).getTime() < new Date(dateFrom + 'T00:00:00+05:30').getTime()) return false
      }
      if (dateTo) {
        if (new Date(e.created_at).getTime() > new Date(dateTo + 'T23:59:59.999+05:30').getTime()) return false
      }
      return true
    })
